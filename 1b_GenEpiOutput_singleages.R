#-----------------------------------------------
# Generate Epi Output for single ages
# Rebecca Clark
...
suppressPackageStartupMessages({
  ...
  require(tidyverse)
  library(data.table)
  source(here("R", "run_param_set_aTB_singleages.R"))
})
...
grid_task_int <- 1 #comment this out on cluster and uncomment line above

vx_scenarios  <- fread("./processing_files/vx_scenario_singleages.csv")
parameters    <- fread(paste0("./processing_files/param_sets/baseline_final.csv"))
parameters <- parameters[sample(.N, 1)]
print(paste0("number of parameter sets to run = ", nrow(parameters)))

if (grid_task_int == 1){
  dir.create("./epi_output/")
  dir.create("./epi_output/n_epi_singleages/")
}
...
for (j in 1:nrow(parameters)) {
  ...
  if (paste0(params_uid, ".parquet") %in% list.files("epi_output/n_epi_singleages/")){
    ...
    write_parquet(rbindlist(cc_n_epi_param), paste0("./epi_output/n_epi_singleages/", params_uid, ".parquet"))
    rm(cc_n_epi_param)
